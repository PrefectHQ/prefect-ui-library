    ...
    >
      ...
      <template #value="{ row }">
        <div class="variables-table__value">
          <VariableV2DisplayPreview :variable="row" value-overflow-text="click to view" @update="handleUpdate" />
        </div>
      </template>
      ...
      <template #action="{ row }">
        <div :key="row.id" class="variables-table__action">
          <VariableV2Menu :variable="row" size="xs" @delete="refreshSubscriptions" @update="handleUpdate" />
        </div>
      </template>
...
<script lang="ts" setup>
  ...
  const columns: TableColumn<VariableV2>[] = [
    {
      property: 'name',
      label: 'Name',
      width: '192px',
  ...
  const selectedVariables = ref<VariableV2[]>([])

  const variablesSubscription = useSubscription(api.variables.getVariablesV2, [filter])
