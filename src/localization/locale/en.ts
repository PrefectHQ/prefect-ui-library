export const en = {
  ...
  info: {
    ...
    baseJobTemplateJsonMessage: (docsUrl: string) => `This is the JSON representation of the base job template. A work pool\'s job template controls infrastructure configuration for all flow runs in the work pool, and specifies the configuration that can be overridden by deployments. For more information on the structure of a work pool\'s base job template, check out [the docs](${docsUrl})`,
    baseJobTemplateVariablesEmpty: 'This work pool\'s base job template does not have any customizations. To add customizations, edit the base job template directly with the **Advanced** tab.',
    baseJobTemplateDefaults: 'These fields control the default values for the base job template and can be overridden by deployments.',
