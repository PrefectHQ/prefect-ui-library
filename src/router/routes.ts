...
export function createWorkspaceRoutes(config?: CreateWorkspaceRoutesConfig) {
  return {
    ...
    customFlowRunCreate: (flowRunId: string) => ({ name: 'workspace.flow-runs.custom-flow-run', params: { flowRunId, ...config } }) as const,
  ...
  return [
    {
      ...
      children: [
        ...
        {
          name: 'workspace.flow-runs.custom-flow-run',
          path: 'flow-run/:flowRunId/custom',
          component: components.customFlowRunCreate,
        },
      ],
    },
    {
